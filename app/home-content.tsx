...
export default function Home() {
  ...
  const doSearch = useCallback(async () => {
    try {
      const showNumber = parseInt(queryParams.show);
      const from = (parseInt(queryParams.page) - 1) * showNumber;
      const selectedBreedsQuery = selectedBreeds.map((breed) => `breeds=${breed}`).join("&");
      const url = `${api}/dogs/search?size=${showNumber}&from=${from}&sort=${queryParams.sortField.toLowerCase()}:${queryParams.sort.toLowerCase()}${queryParams.breeds.length > 0 ? "&" + selectedBreedsQuery : ""}`;
      console.log(url);
